...
docker-nas is a pure docker container providing the functionality of a NAS (network attached server).
The goal of this project is to provide you a simple standalone solution for a specific problem: NAS in a nutshell.
...
Is it secure?
============
This service will be most likely facing the LAN 24h a day.\
...
- We expose as little as possible to the internet, in this project basically only the SMB port (445) and optionally the sftp port (22) for file transfer (ssh login is blocked).
...
docker run --name *NAME* --restart=always -p 445:445 -p *SFTPPORT*:22 -e MY_USERNAME="*USERNAME*" -e MY_PASSWORD="*PASSWORD*" haraldegger/docker-nas
...
- Summarizing the above you will get the following command: *docker run --name mynas --restart=always -p 445:445 -p 10022:22 -e MY_USERNAME="jamesbond006" -e MY_PASSWORD="5RL8~BaJCjw}bW48LL7,wM;P4b7z[7)D,bWctj$0g-y.qN>K,<" haraldegger/docker-nas*
...
- */srv/data/cert/*: This folder contains the self signed SSL certificate for your secure connections.
- */srv/data/cfg/*: This folder contains all configuration files, for example the config file of the samba server.
...
- */srv/data/www/*: This folder contains your data, simply upload your data here and your good to go.
